...
import { insertContactSubmissionSchema, insertClientSchema, insertClientSchemaStep2, clientLoginSchema, adminLoginSchema, insertUserSchema, insertPetSchema, updatePetSchema } from "../shared/schema.js";
...
import { paymentCaptureSchema, paymentCancelSchema } from "../shared/schema.js";
...
export async function registerRoutes(app) {
    ...
    app.get("/admin/api/dashboard/all", async (req, res) => {
        try {
            ...
            const [allGuides, networkUnits, clients, contactSubmissions, plans, allPets] = await Promise.all([
                ...
                if (startDateTime || endDateTime) {
                    filteredGuides = allGuides.filter(guide => {
                        ...
                        if (startDateTime && createdAt < startDateTime)
                            return false;
                        if (endDateTime && createdAt > endDateTime)
                            return false;
                        return true;
                    });
                }
    ...
    app.get("/admin/api/clients/search/:query", async (req, res) => {
        try {
            ...
            const clients = allClients.filter(client => (client.full_name || '').toLowerCase().includes(query) ||
    ...
    app.get("/admin/api/guides/with-network-units", async (req, res) => {
        try {
            ...
            if (search && search.trim()) {
                ...
                filteredGuides = filteredGuides.filter(guide => (guide.procedure && guide.procedure.toLowerCase().includes(searchTerm)) ||
            ...
            if (startDate) {
                ...
                filteredGuides = filteredGuides.filter(guide => guide.createdAt && new Date(guide.createdAt) >= start);
            }
            if (endDate) {
                const end = new Date(endDate);
                end.setHours(23, 59, 59, 999);
                filteredGuides = filteredGuides.filter(guide => guide.createdAt && new Date(guide.createdAt) <= end);
            }
    ...
    app.post("/api/webhooks/cielo", express.raw({ type: 'application/json' }), async (req, res) => {
        ...
        try {
            ...
            const signature = req.headers['cielo-signature'] ||
                req.headers['x-cielo-signature'] || '';
    ...
    app.post("/api/checkout/process", async (req, res) => {
        try {
            ...
            const { clientId, addressData, paymentData, planData, paymentMethod, // 'credit_card', 'pix'
            isRenewal, renewalContractId } = req.body;
    ...
    app.get("/api/clients/procedures", requireClient, async (req, res) => {
        try {
            ...
            const proceduresWithDetails = procedures.map(procedure => {
                ...
                const formatCoparticipationValue = (value) => {
                ...
                const formatCoveragePercentage = (value) => {
                    if (!value || value === 0) {
                        return 0;
                    }
                    return Number(value);
    ...
    app.put("/api/clients/profile", requireClient, async (req, res) => {
        try {
            ...
            if (complement !== undefined)
                updateData.complement = complement;
    ...
    app.get("/api/customer/services", requireClient, async (req, res) => {
        try {
            ...
            const services = await Promise.all(planProcedures.map(async (pp) => {
    ...
    app.get("/api/customer/coparticipation", requireClient, async (req, res) => {
        try {
            ...
            const activeContract = contracts.find(c => c.status === 'active') || contracts[0];
            const planProcedures = await storage.getPlanProcedures(activeContract.planId);
            const coparticipationServices = await Promise.all(planProcedures
        ...
        try {
            ...
            res.json({
                ...
                data: {
                    ...
                    // Cielo payment data
                    cieloStatus: queryResult.payment?.status || 0,
