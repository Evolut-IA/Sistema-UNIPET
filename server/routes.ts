...
import { insertClientSchema, insertPetSchema, insertPlanSchema, insertNetworkUnitSchema, insertFaqItemSchema, insertContactSubmissionSchema, insertSiteSettingsSchema, insertThemeSettingsSchema, insertGuideSchema, insertUserSchema, updateNetworkUnitCredentialsSchema, insertProcedureSchema, insertProcedurePlanSchema, insertNetworkUnitProcedureSchema, type InsertUser } from "@shared/schema";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Network Unit Procedures routes - relacionamento entre unidades da rede e procedimentos
  app.get("/api/network-unit-procedures/:id", async (req, res) => {
    try {
      const networkUnitProcedure = await storage.getNetworkUnitProcedure(req.params.id);
      if (!networkUnitProcedure) {
        return res.status(404).json({ message: "Network unit procedure not found" });
      }
      res.json(networkUnitProcedure);
    } catch (error) {
      res.status(500).json({ message: "Failed to fetch network unit procedure" });
    }
  });

  app.get("/api/network-units/:networkUnitId/procedures", async (req, res) => {
    try {
      const networkUnitProcedures = await storage.getNetworkUnitProcedures(req.params.networkUnitId);
      res.json(networkUnitProcedures);
    } catch (error) {
      res.status(500).json({ message: "Failed to fetch network unit procedures" });
    }
  });

  app.get("/api/procedures/:procedureId/network-units", async (req, res) => {
    try {
      const procedureNetworkUnits = await storage.getProcedureNetworkUnits(req.params.procedureId);
      res.json(procedureNetworkUnits);
    } catch (error) {
      res.status(500).json({ message: "Failed to fetch procedure network units" });
    }
  });

  app.post("/api/network-unit-procedures", async (req, res) => {
    try {
      const networkUnitProcedureData = insertNetworkUnitProcedureSchema.parse(req.body);
      const networkUnitProcedure = await storage.createNetworkUnitProcedure(networkUnitProcedureData);
      res.status(201).json(networkUnitProcedure);
    } catch (error) {
      res.status(400).json({ message: "Invalid network unit procedure data" });
    }
  });

  app.post("/api/network-unit-procedures/bulk", async (req, res) => {
    try {
      const { networkUnitProcedures } = req.body;
      if (!Array.isArray(networkUnitProcedures)) {
        return res.status(400).json({ message: "networkUnitProcedures must be an array" });
      }
      
      const validatedProcedures = networkUnitProcedures.map(procedure => insertNetworkUnitProcedureSchema.parse(procedure));
      const result = await storage.bulkCreateNetworkUnitProcedures(validatedProcedures);
      res.status(201).json(result);
    } catch (error) {
      res.status(400).json({ message: "Invalid network unit procedures data" });
    }
  });

  app.put("/api/network-unit-procedures/:id", async (req, res) => {
    try {
      const networkUnitProcedureData = insertNetworkUnitProcedureSchema.partial().parse(req.body);
      const networkUnitProcedure = await storage.updateNetworkUnitProcedure(req.params.id, networkUnitProcedureData);
      if (!networkUnitProcedure) {
        return res.status(404).json({ message: "Network unit procedure not found" });
      }
      res.json(networkUnitProcedure);
  ...
  app.delete("/api/network-unit-procedures/:id", async (req, res) => {
    try {
      const deleted = await storage.deleteNetworkUnitProcedure(req.params.id);
      if (!deleted) {
        return res.status(404).json({ message: "Network unit procedure not found" });
      }
      res.status(204).send();
    } catch (error) {
      res.status(500).json({ message: "Failed to delete network unit procedure" });
    }
  });

  app.put("/api/network-units/:networkUnitId/procedures", async (req, res) => {
    try {
      const { networkUnitProcedures } = req.body;
      if (!Array.isArray(networkUnitProcedures)) {
        return res.status(400).json({ message: "networkUnitProcedures must be an array" });
      ...
      // Validate each network unit procedure
      const validatedProcedures = networkUnitProcedures.map(procedure => 
        insertNetworkUnitProcedureSchema.parse({
          ...procedure,
          networkUnitId: req.params.networkUnitId
        })
      );
      
      // Use atomic bulk update method
      const result = await storage.bulkUpdateNetworkUnitProceduresForUnit(req.params.networkUnitId, validatedProcedures);
      res.json(result);
    } catch (error) {
      console.error("Error updating network unit procedures atomically:", error);
      if (error instanceof Error && error.name === "ZodError") {
        res.status(400).json({ message: "Invalid network unit procedures data", details: error.message });
      } else {
        res.status(500).json({ message: "Failed to update network unit procedures" });
      }
    }
  });

  app.put("/api/procedures/:procedureId/network-units", async (req, res) => {
    try {
      const { networkUnitProcedures } = req.body;
      if (!Array.isArray(networkUnitProcedures)) {
        return res.status(400).json({ message: "networkUnitProcedures must be an array" });
      ...
      // Use atomic bulk update method
      const result = await storage.bulkUpdateNetworkUnitProceduresForProcedure(req.params.procedureId, validatedProcedures);
      res.json(result);
    } catch (error) {
      console.error("Error updating procedure network units atomically:", error);
      if (error instanceof Error && error.name === "ZodError") {
        res.status(400).json({ message: "Invalid network unit procedures data", details: error.message });
      } else {
        res.status(500).json({ message: "Failed to update procedure network units" });
      }
    }
  });
