...
export class CieloWebhookService {
     ...
     */
    validateWebhookSignature(payload, signature, correlationId) {
     ...
     */
    async processWebhookNotification(notification, correlationId) {
        try {
     ...
     */
    async handlePaymentStatusChange(notification, correlationId) {
        try {
            ...
            await this.updatePaymentStatusInDatabase(notification.PaymentId, notification.ClientOrderId || '', paymentDetails.payment?.status || 0, paymentDetails.payment?.returnCode || '', paymentDetails.payment?.returnMessage || '', correlationId);
     ...
     */
    async handleRecurrencyNotification(notification, correlationId) {
     ...
     */
    async handleChargebackNotification(notification, correlationId) {
     ...
     */
    async handlePaymentApproved(notification, paymentDetails, correlationId) {
     ...
     */
    async handlePaymentDeclined(notification, paymentDetails, correlationId) {
     ...
     */
    async handlePaymentCancelled(notification, paymentDetails, correlationId) {
     ...
     * Update payment status in database
     */
    async updatePaymentStatusInDatabase(paymentId, clientOrderId, status, returnCode, returnMessage, correlationId) {
        try {
     ...
     * Generate official payment receipt automatically
     */
    async generatePaymentReceipt(notification, paymentDetails, correlationId) {
        try {
            ...
            if (!clientData && notification.ClientOrderId) {
                try {
                    ...
                    clientData = clients.find(c => notification.ClientOrderId?.includes(c.email) ||
     ...
     */
    logSecurityAuditEvent(event, details, correlationId) {
        const auditLog = {
            timestamp: new Date().toISOString(),
            event,
