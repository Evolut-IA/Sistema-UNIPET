...
const planProcedures = {
  ...
  "INFINITY": {
    ...
    "Cirurgias": [
      // Este benefício está vazio no relatório, mas vou manter a estrutura
    ],
    "Exames Laboratoriais Complexos": [
...
async function insertProcedures() {
  try {
    // Primeiro, vamos buscar os planos existentes
    ...
    for (const plan of plans) {
      ...
      if (planProcedures[planName]) {
        // Limpar procedimentos existentes
        try {
          await fetch(`http://localhost:3000/api/plan-procedures/${plan.id}`, {
            method: 'DELETE'
          });
        ...
        for (const [benefitName, procedures] of Object.entries(planProcedures[planName])) {
          for (const procedureName of procedures) {
            if (procedureName.trim()) {
              try {
                await fetch('http://localhost:3000/api/plan-procedures', {
                  method: 'POST',
                  headers: {
