...
// Lista de procedimentos para inserir
const procedures = [
...
async function insertProcedures() {
  // Obter o URL base do servidor
  const baseUrl = process.env.REPLIT_DOMAIN ? `https://${process.env.REPLIT_DOMAIN}` : 'http://localhost:5000';
  
  console.log(`Inserindo ${procedures.length} procedimentos...`);
  
  let successCount = 0;
  let errorCount = 0;
  
  for (let i = 0; i < procedures.length; i++) {
    const procedureName = procedures[i];
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`✓ Inserido: ${procedureName}`);
        successCount++;
      } else {
        const error = await response.text();
        console.error(`✗ Erro ao inserir ${procedureName}: ${error}`);
        errorCount++;
      }
    } catch (error) {
      console.error(`✗ Erro de rede ao inserir ${procedureName}:`, error);
      errorCount++;
    }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  insertProcedures().catch(console.error);
}

export { insertProcedures, procedures };