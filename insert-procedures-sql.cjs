...
async function insertProcedures() {
  try {
    ...
    const existingPlans = await sql`SELECT id, name FROM plans`;
    ...
    for (const plan of existingPlans) {
      ...
      if (planProceduresData[planName]) {
        ...
        try {
          await sql`DELETE FROM plan_procedures WHERE plan_id = ${plan.id}`;
        ...
        for (const [benefitName, procedures] of Object.entries(planProceduresData[planName])) {
          for (const procedureName of procedures) {
            if (procedureName.trim()) {
              try {
                await sql`
                  INSERT INTO plan_procedures (plan_id, benefit_name, procedure_name, description, price, is_included, display_order)
                  VALUES (${plan.id}, ${benefitName}, ${procedureName}, '', 0, true, 0)
                `;
                procedureCount++;
              } catch (error) {
