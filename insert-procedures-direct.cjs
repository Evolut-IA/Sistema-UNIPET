...
// Importar o schema
const { plans, planProcedures } = require('./shared/schema.ts');
...
const db = drizzle(sql, { schema: { plans, planProcedures } });
...
async function insertProcedures() {
  try {
    ...
    console.log('Planos encontrados:', existingPlans.map(p => ({ id: p.id, name: p.name })));
    ...
    for (const plan of existingPlans) {
      ...
      if (planProceduresData[planName]) {
        ...
        try {
          await db.delete(planProcedures).where(eq(planProcedures.planId, plan.id));
